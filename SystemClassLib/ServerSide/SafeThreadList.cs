...
{
    [Serializable()]
    public class SafeThreadList<T> : IList<T>
    {
        private List<T> _list = new List<T>();
        private object _sync = new object();
        protected bool _IsReadOnly;

        public SafeThreadList()
        {

        }
        public SafeThreadList(IEnumerable<T> collection)
        {
            lock (_sync)
            {
        ...
        private class ThreadSafeEnumerator : IEnumerator<T>
        {
            ...
            /// </summary>
            private readonly SafeThreadList<T> collection;
            ...
            public ThreadSafeEnumerator(SafeThreadList<T> collection)
            {
                lock (collection._sync)
                {
                    // Make snapshot of passed in collection.
                    this.collection = new SafeThreadList<T>();
